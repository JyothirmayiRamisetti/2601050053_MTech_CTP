...
Start
Display Availability slots: Check and show the current number of available slots.
Read vehicle number: Capture the required details (number plate) for the vehicle.
Check whether parking is available: Check if there is at least one empty slot out of the 100.
Assign or Reject:
